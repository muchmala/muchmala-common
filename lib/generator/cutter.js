...
var SELECT_COVER_COLOR = 'rgba(0,0,255,0.5)';
...
function createPieces(image, settings, callback) {
    var piceSize   = settings.pieceSize,
...
function createFrame(color, filepath, pieceSize, callback) {
    ...
    var iTileCanvas = new Canvas(pieceSize, pieceSize);
    var iTileCtx = iTileCanvas.getContext('2d');
    drawPiecePath(iTileCtx, step, {left: 0, top: 0, right: 0, bottom: 0});
    iTileCtx.fillStyle = color;
    iTileCtx.fill();
    
    var oTileCanvas = new Canvas(pieceSize, pieceSize);
    var oTileCtx = oTileCanvas.getContext('2d');
    drawPiecePath(oTileCtx, step, {left: 1, top: 1, right: 1, bottom: 1});
    oTileCtx.fillStyle = color;
    oTileCtx.fill();
    
    frameCtx.drawImage(oTileCanvas, step, 0, fLength, fWidth,
        0, 0, fLength, fWidth);
    frameCtx.drawImage(oTileCanvas, step, pieceSize - fWidth, fLength, fWidth,
        0, fWidth, fLength, fWidth);
    
    frameCtx.drawImage(iTileCanvas, step, 0, fLength, fWidth,
        0, fWidth * 2, fLength, fWidth);
    frameCtx.drawImage(iTileCanvas, step, pieceSize - fWidth, fLength, fWidth,
        0, fWidth * 3, fLength, fWidth);
        
    frameCtx.drawImage(oTileCanvas, 0, step, fWidth, fLength,
        fLength, 0, fWidth, fLength);
    frameCtx.drawImage(oTileCanvas, pieceSize - fWidth, step, fWidth, fLength,
        fLength + fWidth, 0, fWidth, fLength);
        
    frameCtx.drawImage(iTileCanvas, 0, step, fWidth, fLength,
        fLength, fLength, fWidth, fLength);
    frameCtx.drawImage(iTileCanvas, pieceSize - fWidth, step, fWidth, fLength,
        fLength + fWidth, fLength, fWidth, fLength);

    frameCanvas.toBuffer(function(err, buffer) {
        fs.writeFile(filepath, buffer, callback);
    });
}
...
exports.createFrame = function(size, dirpath, callback) {
    flow.exec(function() {
        fs.mkdir(dirpath, 0777, this);
    }, function() {
        createFrame(FRAME_COLOR, dirpath + '/' + FRAME_FILENAME, size, this);
    }, callback);
};